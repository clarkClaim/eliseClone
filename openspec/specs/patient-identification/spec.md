...
The tool SHALL return patient details when both phone and DOB verification succeed.
...
- **THEN** the tool returns `{ status: "existing", patient: { id, name, ... } }`
...
- **THEN** the tool returns `{ status: "existing", patient: { id, name, ... }, phoneAdded: true }`
