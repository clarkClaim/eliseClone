...
### Requirement: Legacy timeslot-based availability

**Reason:** Bahmni does not use discrete timeslots. Appointments are booked directly with start/end times within service availability windows.

**Migration:** Use `getAvailability()` to get service hours, then book with specific times via `createAppointment()`.

### Requirement: Legacy appointmentscheduling module probe

**Reason:** O3 does not have the legacy module installed.

**Migration:** Module detection now probes Bahmni endpoints.
